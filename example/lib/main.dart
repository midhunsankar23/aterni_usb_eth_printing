...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  ...
  Future<void> _tryAutoConnect(Map<String, dynamic> device) async {
    try {
      print(
        "Attempting auto-connect to ${device['manufacturer']} ${device['productName']}",
      );
      await _connect(
    ...
    try {
      ...
      setState(() {
        ...
        if (result == true) {
          ...
          _statusMessage =
              'Failed to connect. Please check printer and permissions.';
        }
      });
    } on PlatformException catch (e) {
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (!_isLoading)
                              ElevatedButton.icon(
                                onPressed:
                                    () => _getDevicelist(silentMode: false),
                                icon: const Icon(Icons.refresh),
